...
void Renderer::Draw(const GameTimer& gt)
{
    ...
    ThrowIfFailed(mCommandList->Reset(cmdListAlloc.Get(), mPSOs["basic"].Get()));
...
void Renderer::BuildRootSignature()
{
    ...
    CD3DX12_ROOT_PARAMETER slotRootParameter[ROOT_SIGNITURE_SLOT];
	...
	CD3DX12_ROOT_SIGNATURE_DESC rootSigDesc(ROOT_SIGNITURE_SLOT, slotRootParameter,
...
void Renderer::BuildShapeGeometry()
{
	...
	GeometryGenerator::MeshData quad = geoGen.CreateQuad(0, 0.5, 0.5, 0.5, 1);
	...
	indicies.reserve(totalIndexCount);

	indicies.insert(indicies.end(), std::begin(box.GetIndices16()), std::end(box.GetIndices16()));
	indicies.insert(indicies.end(), std::begin(grid.GetIndices16()), std::end(grid.GetIndices16()));
	indicies.insert(indicies.end(), std::begin(sphere.GetIndices16()), std::end(sphere.GetIndices16()));
	indicies.insert(indicies.end(), std::begin(cylinder.GetIndices16()), std::end(cylinder.GetIndices16()));
	indicies.insert(indicies.end(), std::begin(quad.GetIndices16()), std::end(quad.GetIndices16()));
...
void Renderer::LoadAnimationModel(const char * path, std::string name)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		vertices.push_back({ position, normal,uv,{0.0f,0.0f,0.0f},{0,0,0,0}});
	}
...
void Renderer::BuildPSOs()
{
    ...
    ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&opaquePsoDesc, IID_PPV_ARGS(&mPSOs["basic"])));
	...
	firePsoDesc.DepthStencilState.DepthEnable = false;
