...
{
	for (int i = 0; i < m_nObjects; i++) m_ppObjects[i]->m_pObjectCollided = NULL;//모든 오브젝트의 충돌 레퍼런스를 NULL로 만들고 계산
	//10x10 행렬이라면 자기 자신과는 안해도 되니까 대각선 기준으로 위쪽만 하면 된다.
	for (int i = 0; i < m_nObjects; i++)//충돌한 놈을 찾아서 속도를 교환하겠다. 동시에 두개 이상이 부딫힐 수도 있다.
	{
	...
	for (int i = 0; i < m_nObjects; i++)//각각에 오브젝트에 대해서 상대방이 있으면 서로의 속도를 교환할 꺼다.
	{
...
void CCollisionScene::CheckObjectByWallCollisions()//어느 벽을 뚫었는지 찾아서 그 점에서 반사가 일어나게하겠다.
{
